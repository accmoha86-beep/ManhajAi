...
export default function TopBar() {
  ...
  return (
    <header
      className="fixed top-0 left-0 right-0 h-16 z-[1001] flex items-center justify-between px-5"
      style={{
    ...
    >
      ...
      <div className="flex items-center gap-2">
        ...
        <Link
          href="/settings"
          className={topBarBtnClass(pathname === '/settings')}
        >
          <Settings size={15} />
          <span>الإعدادات</span>
        </Link>
        ...
        {!isAuthenticated ? (
          <>
            <Link
              href="/login"
              className={topBarBtnClass(pathname === '/login')}
            >
              <LogIn size={15} />
            ...
            <Link
              href="/register"
              className={topBarBtnClass(pathname === '/register')}
            >
              <UserPlus size={15} />
